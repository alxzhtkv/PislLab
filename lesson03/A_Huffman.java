    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (int i=0; i<s.length(); i++){
            if (!count.containsKey(s.charAt(i)))
                count.put(s.charAt(i),1); //добавляем в map значения (букву), если его пока там нет
            else
                count.replace(s.charAt(i), count.get(s.charAt(i))+1);  //перемещаем в map значения (букву)
        }
